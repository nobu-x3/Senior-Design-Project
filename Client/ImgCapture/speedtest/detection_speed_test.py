...
#--------------------------------------------
def detect_face(detector_mode:str):
	quit = False
	tic = time.time()
	frame = 0
	x_out = []
...
def writefile(name:str, data:np.array):
...
def study(model:str):
	...
	x, y = detect_face(model)
	a = np.stack((x, y), axis=-1)
	print(a)
	writefile(model + '.csv', a)
...
if __name__ == "__main__":
	...
	detector_model = detector_models[4]


	#--------------------------------------------

	cap = cv2.VideoCapture('demo_long.mp4')

	study(detector_model)